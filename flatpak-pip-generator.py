...
def get_tar_package_url_pypi(name: str, version: str) -> str:
    ...
    with urllib.request.urlopen(url) as response:  # noqa: S310
        ...
        # Prefer manylinux wheels for specific packages that are hard to build
        if name in ["maturin", "cryptography"]:
            for source in body["urls"]:
                if "manylinux" in source["url"] and "x86_64" in source["url"] and source["url"].endswith(".whl"):
                    return str(source["url"])
...
with tempfile.TemporaryDirectory(prefix=tempdir_prefix) as tempdir:
    ...
    for filename in os.listdir(tempdir):
        if not filename.endswith(("bz2", "any.whl", "gz", "xz", "zip", ".whl")):
