...
try:
    ...
    print("Critical Error: 'cryptography' module not found.")
...
TRANSLATIONS = {
    'English': {
        ...
        "menu_edit_bill": "Edit Bill",
        "menu_delete_bill": "Delete Bill",
        "menu_restore_unpaid": "Restore to Unpaid",
        ...
        "msg_confirm_disable_pin": "Enter current PIN to disable protection:",
    ...
    'Georgian': {
        ...
        "btn_clear_all_data": "ყველაფრის წაშლა",
        ...
        "menu_edit_bill": "რედაქტირება",
        "menu_delete_bill": "წაშლა",
        "menu_restore_unpaid": "გადატანა გადასახდელში",
        "menu_delete_permanently": "სამუდამოდ წაშლა",
        ...
        "msg_confirm_disable_pin": "შეიყვანეთ მიმდინარე PIN-კოდი დაცვის გამოსართავად:",
...
def get_icon(name):
    """Helper to get QIcon from icons folder."""
    return QIcon(resource_path(f'icons/{name}.svg'))
...
class DataManager:
    ...
    def save_data(self, data_to_save, pin=None):
        ...
        try:
            ...
            # 2. Encryption or Plain JSON (v7.4.0 Hardening)
            if pin:
                fernet = self._get_fernet(pin)
                final_data = fernet.encrypt(json_str.encode('utf-8'))
                write_mode = 'wb'
            else:
                # Save as plain JSON if no PIN (User disabled protection)
            ...
            for i in range(retries):
                try:
                    ...
                    logging.error(f"Error during atomic swap: {e}")
                    raise e
...
class SettingsDialog(QDialog):
    ...
    def save_settings(self):
        ...
        # v7.4.0: Requirement for PIN verification when disabling protection
        old_pin_enabled = self.data_manager.security.get('pin_enabled', False)
        pin_enabled = self.pin_chk.isChecked()
        
        if old_pin_enabled and not pin_enabled:
            # Transition: Protection -> Disabled
            pin_hint = self.data_manager.security.get('pin_hint')
            dialog = PinEntryDialog(self, mode='verify', hint=pin_hint)
            dialog.label.setText(STRINGS["msg_confirm_disable_pin"])
            
            if dialog.exec() == QDialog.DialogCode.Accepted:
                entered_pin = dialog.get_pin()
                stored_hash = self.data_manager.security.get('pin_hash')
                
                # Verify PIN (Session or Hash)
                is_valid = False
                if self.session_pin:
                ...
                if not is_valid:
                    QMessageBox.warning(self, STRINGS["title_invalid_pin"], STRINGS["msg_attempts_remaining"].format(count=0))
                    self.pin_chk.setChecked(True)
                    return
                
                # Correct PIN - Clear session and DECRYPT data files to plain JSON
                try:
                    if hasattr(self.parent(), 'session_pin'):
                        self.parent().session_pin = None
                    self.session_pin = None
                    
                    # Decrypt main bill data file immediately
                    current_data = self.data_manager.load_data(entered_pin)
                    self.data_manager.save_data(current_data, pin=None)
                    
                    # Security metadata will follow updated 'pin_enabled' flag
                except Exception as e:
                    QMessageBox.critical(self, STRINGS["title_error"], f"Decryption failed: {e}")
                    self.pin_chk.setChecked(True)
                    return
            else:
                # Cancelled
    ...
    def setup_pin(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            # Update config (Sync with current UI state to avoid theme jumping 🎨)
            ...
            # Update session pin for the current session (Parent and current Dialog)
            if hasattr(self.parent(), 'session_pin'):
                self.parent().session_pin = new_pin
            self.session_pin = new_pin # Ensure current dialog session uses new PIN for further saves
...
class ChartWidget(QWidget):
    ...
    def contextMenuEvent(self, event):
        menu = QMenu(self)
        save_action = menu.addAction(get_icon("save"), STRINGS["legend_save_image"])
...
class BillTrackerWindow(QMainWindow):
    ...
    def __init__(self, session_pin=None):
        ...
        try:
            ...
            self.tabs.addTab(self.dashboard_tab, get_icon("chart-bar"), STRINGS["tab_dashboard"])
            
            # Tab 2: Unpaid Bills
            self.unpaid_tab = QWidget()
            self.setup_unpaid_tab()
            self.tabs.addTab(self.unpaid_tab, get_icon("money-bill"), STRINGS["tab_unpaid"])
            
            # Tab 3: Paid History
            self.paid_tab = QWidget()
            self.setup_paid_tab()
            self.tabs.addTab(self.paid_tab, get_icon("clipboard"), STRINGS["tab_paid"])

            # Tab 4: Charts
            self.chart_tab = QWidget()
            self.setup_chart_tab()
            self.tabs.addTab(self.chart_tab, get_icon("chart-bar"), STRINGS["tab_trends"])

            # Tab 5: Calendar
            self.calendar_tab = CalendarTab(self)
            self.tabs.addTab(self.calendar_tab, get_icon("calendar"), STRINGS["tab_calendar"])
            self.calendar_tab.refresh_data()

            # Tab 6: Subscriptions
            self.subscription_tab = SubscriptionTab(self)
            self.tabs.addTab(self.subscription_tab, get_icon("credit-card"), STRINGS["tab_subscriptions"])

            # Tab 7: Savings Goals (v6.7.0)
            self.savings_tab = SavingsTab(self)
            self.tabs.addTab(self.savings_tab, get_icon("target"), STRINGS["tab_savings"])

            # Tab 8: About
            ma_about_text = STRINGS.get("tab_about", "ℹ️ About")
            self.about_tab = QWidget()
            self.setup_about_tab()
            self.tabs.addTab(self.about_tab, get_icon("info-circle"), ma_about_text)
    ...
    def setup_dashboard_tab(self):
        ...
        export_btn.setIcon(get_icon("file-csv"))
        export_btn.clicked.connect(self.export_csv)
        btn_layout.addWidget(export_btn)
        
        pdf_btn = QPushButton(STRINGS["btn_export_pdf"])
        pdf_btn.setIcon(get_icon("file-pdf"))
        ...
        converter_btn.setIcon(get_icon("money-bill"))
        ...
        settings_btn.setIcon(get_icon("settings"))
        settings_btn.clicked.connect(self.open_settings)
        actions_layout.addWidget(settings_btn)
        
        tray_btn = QPushButton(STRINGS["btn_minimize_tray"])
        tray_btn.setIcon(get_icon("minimize"))
    ...
    def setup_chart_tab(self):
        ...
        export_pdf_btn.setIcon(get_icon("file-pdf"))
    ...
    def setup_about_tab(self):
        ...
        self.contact_btn.setIcon(get_icon("mail"))
        ...
        self.donate_btn.setIcon(get_icon("credit-card"))
    ...
    def show_unpaid_context_menu(self, position):
        ...
        pay_action = menu.addAction(get_icon("money-bill"), pay_label)
        # Edit only allowed for single selection
        edit_action = menu.addAction(get_icon("edit"), STRINGS["menu_edit_bill"]) if not is_multiple else None
        delete_action = menu.addAction(get_icon("trash"), delete_label)
    ...
    def show_paid_context_menu(self, position):
        ...
        menu = QMenu()
        restore_action = menu.addAction(get_icon("undo"), restore_label)
        delete_action = menu.addAction(get_icon("trash"), delete_label)
